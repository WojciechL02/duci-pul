...
# Import flash_attn's attention (with fallback to PyTorch SDPA)
try:
    ...
    _w.warn("flash_attn not installed; falling back to PyTorch scaled_dot_product_attention")
    def flash_attn_func(q, k, v, dropout_p=0.0, softmax_scale=None, **kwargs):
        # q,k,v: (B, L, H, d) -> need (B, H, L, d) for F.scaled_dot_product_attention
        import torch.nn.functional as _F
        q2 = q.transpose(1, 2)
        k2 = k.transpose(1, 2)
        v2 = v.transpose(1, 2)
        out = _F.scaled_dot_product_attention(q2, k2, v2, dropout_p=dropout_p, scale=softmax_scale)
        return out.transpose(1, 2)  # back to (B, L, H, d)
    def flash_attn_varlen_kvpacked_func(q, kv, cu_seqlens_q, cu_seqlens_k, max_seqlen_q, max_seqlen_k, dropout_p=0.0, softmax_scale=None):
        ...
        for i in range(B):
            ...
            qi = q[sq:eq].unsqueeze(0).transpose(1, 2)   # (1, H, Lq, d)
            ki = k[sk:ek].unsqueeze(0).transpose(1, 2)   # (1, H, Lk, d)
            vi = v[sk:ek].unsqueeze(0).transpose(1, 2)   # (1, H, Lk, d)
            oi = _F.scaled_dot_product_attention(qi, ki, vi, dropout_p=dropout_p, scale=softmax_scale)
            outputs.append(oi.transpose(1, 2).squeeze(0))  # (Lq, H, d)
