...
# ---------------------------------------------------------------------------
def process_split(infinity, vae, text_cond_b1,
    ...
    for start in range(0, total, ae_bs):
        ...
        for j in range(0, len(pil_images), gen_bs):
            ...
            synth_np = batch_gen_img2img(
                infinity, vae, text_cond_b1, sub_bits,
...
def main():
    ...
    # Encode prompt once (B=1), will be replicated per batch
    text_cond_b1 = encode_prompt(text_tokenizer, text_encoder, args.prompt)

    if not args.skip_tp:
        process_split(infinity, vae, text_cond_b1,
