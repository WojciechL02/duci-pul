...
def features_extraction(
    ...
    if attack_cfg.name == "image_paths":
        model = None
    else:
...
def scores_computation(
    ...
    features, _ = load_data(computer, config.path_to_features)
    computer.run(features)
...
def evaluation(
    ...
    members_scores, nonmembers_scores = load_members_nonmembers_scores(
        config, model_cfg, action_cfg, attack_cfg, dataset_cfg
    )
