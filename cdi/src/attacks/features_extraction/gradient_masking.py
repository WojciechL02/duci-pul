...
class GradientMaskingExtractor(FeatureExtractor):
    ...
    def get_masks(self, gradients: T, B: int) -> T:
        ...
        for gradient in gradients.permute(1, 0, 2, 3, 4):  # iterate over timesteps
            # `.reshape` instead of `.view` — gradients from some transformer
            # backbones (e.g. NitroT MMDiT's final einsum) come back with
            # non-contiguous strides, which `.view` rejects.
            _, i = torch.topk(gradient.reshape(B, -1).abs(), k=k, dim=1)
