...
class FeatureExtractor(DataSource):
    ...
    def normalize_batch(batch):
        """
        Ensure the batch returned by a DataLoader is at least a 2-tuple: (images, targets).
        When the dataloader returns a third element (image paths), it is preserved.
        Returns (images, targets) or (images, targets, paths).
        """
    ...
    def process_data(self, *args, **kwargs) -> T:
        ...
        for batch in tqdm(loader, total=total_batches):
            ...
            images, targets = normalized[0], normalized[1]
