...
# main.py uses relative `sys.path.append("./U-ViT" / "./latent-diffusion" / "./DiT_MoE")`
# so it must be launched from cdi/, not from wherever the user's shell happens to be.
cd "$CDI_DIR"
...
run_cpu_job() {
    ...
    # Note: keep at least one GPU visible (CUDA_VISIBLE_DEVICES=0) even though
    # action.device=cpu, because module-level imports in some model wrappers
    # (e.g. MinFMWrapper -> knapformer) call `torch.cuda.get_device_name()`
    # at import time and crash with `No CUDA GPUs are available` on a fully
    # masked GPU set. action.device=cpu still ensures the actual scoring math
    # runs on the CPU.
    CUDA_VISIBLE_DEVICES="${CPU_PHASE_GPU:-0}" \
        "$P" -u "$CDI_DIR/main.py" \
