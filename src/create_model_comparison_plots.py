...
for i, (model_name, files) in enumerate(sorted(model_files.items())):
    ...
    method_data = defaultdict(lambda: {'p_values': [], 'p_hat_values': []})
    ...
    for file in sorted(files, key=lambda x: parse_filename(x)[1]):
        ...
        try:
            ...
            # Extract p_hat values for each method
            for _, row in df.iterrows():
                method = row['method']
                p_hat = extract_mean(row['p_hat_mean_std'])

                method_data[method]['p_values'].append(p_value)
                method_data[method]['p_hat_values'].append(p_hat)
        except Exception as e:
    ...
    for method, data in sorted(method_data.items()):
        if len(data['p_values']) > 0:
            ...
            p_hat_values = np.array(data['p_hat_values'])
            sort_idx = np.argsort(p_values)

            ax.plot(p_values[sort_idx], p_hat_values[sort_idx], marker='o', 
    ...
    ax.set_ylabel('Estimated p (p_hat)', fontsize=14)
...
fig.suptitle('Comparison of Methods Across Different Models', fontsize=20, fontweight='bold', y=0.99)

# Save the figure
plt.savefig('../results/model_comparison_plots.png', dpi=300, bbox_inches='tight')
plt.savefig('../results/model_comparison_plots.pdf', bbox_inches='tight')

print("Plots created and saved to '../results/model_comparison_plots.png' and '../results/model_comparison_plots.pdf'")
