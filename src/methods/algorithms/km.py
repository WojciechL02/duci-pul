...
class KM:
    """
    Implementation of the method presented in "Mixture Proportion Estimation via Kernel Embedding of Distributions," avXiv:1603.02501.
    """

    def __init__(self, stability_eps: float = 1e-3) -> None:
        self.stability_eps = stability_eps

    def find_nearest_valid_distribution(self, u_alpha, kernel, initial=None, reg=0):
        ...
        n = kernel.shape[0]
        P_numpy = 2 * kernel + (2 * reg + self.stability_eps) * np.eye(n)
        P = matrix(P_numpy)
        q = matrix(np.dot(-2 * kernel, u_alpha))
        if np.isnan(kernel).any():
            raise ValueError(
                "Kernel matrix contains NaNs. Check your input data X_mixture/X_component for NaNs or Infs."
            )
        ...
        distance_sqd = (
            ...
            + np.dot(u_alpha.T, np.dot(kernel, u_alpha))[0, 0]
        )
    ...
    def get_distance_curve(
        ...
        for lambda_value in lambda_values:
            ...
            ) + (1 - lambda_value) / M * np.concatenate(
                (np.zeros((N, 1)), np.ones((M, 1)))
            )
            (solution, distance_sqd) = self.find_nearest_valid_distribution(
            ...
            if distance_sqd >= 0:
                ...
                if distance_sqd < -1e-5:
                    raise ValueError(
                        "They want to take the square root of the negative number!"
                    )
                d_lambda.append(0)
            # ML - end
    ...
    def compute_best_rbf_kernel_width(self, X_mixture, X_component):
        N = X_mixture.shape[0]
    ...
    def mpe(self, kernel, N, M, nu, epsilon=0.04, lambda_upper_bound=8.0):
    ...
    def estimate(self, X, y):
        ...
        N = X_mixture.shape[0]
        M = X_component.shape[0]
        best_width, kernel = self.compute_best_rbf_kernel_width(X_mixture, X_component)
        lambda_values = np.array([1.00, 1.05])
        dists = self.get_distance_curve(kernel, lambda_values, N=N, M=M)
