...
def parse_propensity_attributes(arg):
    ...
    propensity_attributes_signs = list(
        map(lambda s: -1 if "-" in s else 1, propensity_attributes_str)
    )
...
def label(args):
    ...
    if len(args) == 0:
        print(
            "Usage: python -m sarpu label <data_folder> <data_name> <labeling_model_type> <propensity_attributes> <nb_assignments> [relabel] [seed]"
        )

    else:

        data_folder, data_name, labeling_model_type = args[0:3]
        propensity_attributes, propensity_attributes_signs = (
            parse_propensity_attributes(args[3])
        )
...
def train_eval(args):
    if len(args) == 0:
        print(
            "Usage: python -m sarpu train_eval <data_folder> <results_folder> <data_name> <labeling_model_type> <propensity_attributes> <labeling> <partition> <settings> <pu_method> [rerun]"
        )
        ...
        propensity_attributes, propensity_attributes_signs = (
            parse_propensity_attributes(args[4])
        )
...
def outdir(args):
    if len(args) == 0:
        print(
            "Usage: python -m sarpu outdir <data_folder>  <data_name> <labeling_model_type> <propensity_attributes> <labeling> <partition> <settings> <pu_method>"
        )
