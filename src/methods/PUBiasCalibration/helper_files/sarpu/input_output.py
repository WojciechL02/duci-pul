...
def read_data(data_files, partitions_file=None, delimiter=None):
    datas = list(
        map(lambda data_path: np.loadtxt(data_path, delimiter=delimiter), data_files)
    )
    ...
    for partition in sorted(list(set(partitions) - set([0]))):
        data_per_partition.append(
            tuple(map(lambda data: data[partitions == partition], datas))
        )
