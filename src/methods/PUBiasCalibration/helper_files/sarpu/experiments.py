...
from .labeling_mechanisms import parse_labeling_model
from .paths_and_names import *
...
def train_and_evaluate(
    ...
    """
    out_dir = experiment_method_result_folder_path(
        results_folder,
        data_name,
        labeling_model,
        labeling,
    ...
    if rerun_experiments or not (
        ...
        classification_model_type, propensity_model_type, classification_attributes = (
        ...
        s_path = propensity_labeling_path(
            data_folder, data_name, labeling_model, labeling
        )
        e_path = propensity_scores_path(data_folder, data_name, labeling_model)
        f_path = partition_path(data_folder, data_name, partition)
        (x_train, y_train, s_train, e_train), (x_test, y_test, s_test, e_test) = (
            read_data((x_path, y_path, s_path, e_path), f_path)
        )
        ...
        if pu_method == "sar-e":
            ...
            e_model = pickle.load(
                open(
                    propensity_model_path(data_folder, data_name, labeling_model), "rb"
                )
            )

        elif pu_method == "scar-c":
            f_model, info = pu_learn_scar_c(
                x_train,
                s_train,
                c,
                classification_model_type(),
            ...
            f_model, e_model, info = pu_learn_sar_em(
                x_train,
                s_train,
                labeling_model.propensity_attributes,
                classification_model=classification_model_type(),
                classification_attributes=classification_attributes,
                propensity_model=propensity_model_type(),
            )

        elif pu_method == "supervised":
            f_model, info = pu_learn_neg(
                x_train,
                y_train,
                classification_model=classification_model_type(),
                classification_attributes=classification_attributes,
        ...
        out_classifier = experiment_classifier_path(
            results_folder,
            data_name,
            labeling_model,
            labeling,
        ...
        out_propensity_model = experiment_propensity_model_path(
            results_folder,
            data_name,
            labeling_model,
            labeling,
        ...
        results_train = evaluate_all(
            y_train,
            s_train,
            e_train,
            f_model.predict_proba(x_train),
            e_model.predict_proba(x_train),
        )
        ...
        out_results = experiment_results_path(
            results_folder,
            data_name,
            labeling_model,
            labeling,
...
def copy_if_possible(
    results_folder, data_name, labeling_model, labeling, partition, settings, pu_method
):
    ...
    if pu_method != "supervised":
        ...
        try:
            complete_result_path = next(
                glob.iglob(
                    experiment_method_result_path_nolabel(
                        results_folder, data_name, partition, settings, pu_method
                    )
                )
            )
...
def summarize_results(results_folder, experiments_overview_file):
    ...
    df_experiments = pd.read_csv(
        experiments_overview_file, dtype={"propensity_attributes": str}
    )
    ...
    for _, experiment in df_experiments.iterrows():
        try:
            labeling_model = parse_labeling_model(
                ...
                [
                    abs(int(i))
                    for i in str(experiment["propensity_attributes"]).split(".")
                ],
            ...
            df_results = pd.Series.from_csv(
                results_path, index_col=0, header=None, sep="\t"
            )
    ...
    output_file = "{0}_{2}{1}".format(
        *os.path.splitext(experiments_overview_file), "results"
    )
    ...
    output_file = "{0}_{2}{1}".format(
        *os.path.splitext(experiments_overview_file), "failed"
    )
    df.to_csv(output_file, index=False)
