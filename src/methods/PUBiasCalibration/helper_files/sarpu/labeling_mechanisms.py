...
def label_data(
    ...
    labeling_model = parse_labeling_model(
        labeling_model_type, propensity_attributes, propensity_attributes_signs
    )
    ...
    x, y = read_data(
        (data_path(data_folder, data_name), classlabels_path(data_folder, data_name))
    )

    if relabel_data or not os.path.exists(
        propensity_model_path(data_folder, data_name, labeling_model)
    ):
        _save_propensity_model_and_labels(
            data_folder, data_name, x, y, labeling_model, nb_assignments, seed=None
        )

    return labeling_model


def parse_labeling_model(
    labeling_model_type, propensity_attributes, propensity_attributes_signs
):
    ...
    if match:
        ...
        return SimpleLabeling(
            propensity_attributes, propensity_attributes_signs, min_prob, max_prob
        )

    raise ValueError(
        "Could not parse the labeling model type %s " % labeling_model_type
    )


def _save_propensity_model_and_labels(
    data_folder, data_name, x, y, propensity_model, nb_assignments=10, seed=None
):
    ...
    for i in range(nb_assignments):
        out_labeling = propensity_labeling_path(
            data_folder, data_name, propensity_model, i
        )
...
class BaseLabelingMechanism:
    ...
    def name(self):
        return self.type_name() + "._." + ".".join(map(str, self.propensity_attributes))
...
class SimpleLabeling(BaseLabelingMechanism):
    ...
    def propensity_scores(self, x):
        ...
        x_e = x[:, np.abs(self.propensity_attributes)] * np.sign(
            self.propensity_attributes
        )
        # Compute the propensity score of each instance
        scaled = self.min_prob + (x_e - self.minx) / (self.maxx - self.minx) * (
