...
def data_path(data_folder, data_name):
    return os.path.join(
        processed_data_path(data_folder, data_name), data_fname(data_name)
    )
...
def classlabels_path(data_folder, data_name):
    return os.path.join(
        processed_data_path(data_folder, data_name), classlabels_fname(data_name)
    )
...
def partition_path(data_folder, data_name, partition):
    return os.path.join(
        partitions_data_path(data_folder, data_name),
        partition_name(data_name, partition),
    )
...
def propensity_base_path(data_folder, data_name, propensity_model):
    return os.path.join(
        labelings_data_path(data_folder, data_name),
        propensity_fname(data_name, propensity_model),
    )
...
def propensity_labeling_path(data_folder, data_name, propensity_model, labeling):
    return (
        propensity_base_path(data_folder, data_name, propensity_model)
        + "._.lab."
        + str(labeling)
        + ".csv"
    )
...
def experiment_result_folder_path(
    results_folder, data_name, propensity_model, labeling, partition, settings
):
    return os.path.join(
        results_folder,
        data_name,
        ".__.".join(
            list(map(str, [propensity_model.name(), settings, labeling, partition]))
        ),
    )


def experiment_method_result_folder_path(
    results_folder,
    data_name,
    propensity_model,
    labeling,
...
def experiment_method_result_path_nolabel(
    ...
    return "/".join(
        [
            results_folder,
            data_name,
            ".__.".join(list(map(str, ["*", settings, "*", str(partition)]))),
            pu_method,
            "results.csv",
        ]
    )
...
def experiment_classifier_path(
    results_folder,
    data_name,
    propensity_model,
    labeling,
    ...
    return os.path.join(
        experiment_method_result_folder_path(
            results_folder,
            data_name,
            propensity_model,
            labeling,
...
def experiment_propensity_model_path(
    results_folder,
    data_name,
    propensity_model,
    labeling,
