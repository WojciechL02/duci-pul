...
def pu_learn_sar_e(x, s, e, classification_model=None, classification_attributes=None):
    ...
    classification_model = LimitedFeaturesModel(
        classification_model, classification_attributes
    )
...
def pu_learn_sar_em(
    ...
    expected_prior_y1 = np.clip(
        expected_prior_y1, EPS, 1 - EPS
    )  # to avoid numerical problems
    expected_propensity = np.clip(
        expected_propensity, EPS, 1 - EPS
    )  # to avoid numerical problems
    ...
    for i in range(max_its):
        ...
        classification_s = np.concatenate(
            [np.ones_like(expected_posterior_y1), np.zeros_like(expected_posterior_y1)]
        )
        ...
        if i > convergence_window:
            max_ll_improvement = (
                max(loglikelihoods[-convergence_window:])
                - loglikelihoods[-convergence_window]
            )
            ...
            if verbose and (i + 1) % 1 == 0:
                print(
                    f"{log_prefix}Iter {i + 1}/{max_its}, time: {time.time()-start:.1f}s, average_abs_slope: {average_abs_slope:.4f}, max_ll_improvement: {max_ll_improvement:.4f}"
                )
        else:
            if verbose and (i + 1) % 1 == 0:
                print(
                    f"{log_prefix}Iter {i + 1}/{max_its}, time: {time.time()-start:.1f}s"
                )
...
def loglikelihood_probs(class_probabilities, propensity_scores, labels):
    ...
    prob_unlabeled_pos = np.clip(
        prob_unlabeled_pos, EPS, 1 - EPS
    )  # introduced to avoid numerical problems in log later
    prob_unlabeled_neg = 1 - class_probabilities
    prob_unlabeled_neg = np.clip(
        prob_unlabeled_neg, EPS, 1 - EPS
    )  # introduced to avoid numerical problems in log later
